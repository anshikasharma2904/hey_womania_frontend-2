    ...
    <>
      <div className="fixed bottom-6 right-0 z-[70] flex items-end gap-3 md:bottom-8">
        ...
        <div className="group mr-5 flex items-center gap-3 md:mr-6">
          <div className="hidden rounded-xl bg-black px-4 py-2 text-sm font-medium text-white opacity-0 shadow-[0_12px_24px_rgba(0,0,0,0.24)] transition-opacity duration-200 group-hover:opacity-100 md:block">
            Latest Offers
          </div>
        <button
          type="button"
          aria-label="Open latest offers"
          onClick={() => setOpen(true)}
          className="group relative flex h-14 w-14 items-center justify-center rounded-full bg-[linear-gradient(180deg,#7f5c4e_0%,#5f5d3e_100%)] text-white shadow-[0_18px_40px_rgba(95,93,62,0.34)] transition-all duration-300 hover:-translate-y-1 hover:scale-[1.03] md:h-16 md:w-16"
        >
          <span className="material-symbols-outlined text-[1.7rem] md:text-[2rem]">
            notifications
          </span>
          <span className="absolute -right-1 -top-1 flex h-6 min-w-6 items-center justify-center rounded-full bg-[#ef6f63] px-1 text-[0.68rem] font-semibold text-white">
            {unreadCount}
          </span>
        </button>
        </div>
